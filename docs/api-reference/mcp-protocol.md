...
All MCP servers expose a `resource:version` endpoint:
...
* Validate all inputs using JSON Schema or typed models (Pydantic, Zod)
* Never execute raw code from input
* Sanitize logs (remove tokens and PII)

---

📎 **See also:**

* [mcp-tools-and-resources.md](mcp-tools-and-resources.md)
* [auth.md](auth.md)
...
Each MCP server must register its tools and resources during startup.
...
All tool invocations follow MCP’s standard error schema.
...
* Prefer **structured** responses over text
* Document input/output with JSON Schemas
* Always return `ok` and `error` fields
* Include example invocations in READMEs
...
3. Sends with each MCP request:

   ```http
   Authorization: Bearer <access_token>
   ```
4. Server validates via introspection endpoint or local verification.
...
| `admin`    | Register/unregister tools and resources |
...
* Always use **HTTPS/TLS** if HTTP transport is enabled
...
✅ These versions are 100% valid Markdown:
- All code fences properly opened/closed.
- Syntax-highlight tags (`json`, `python`, `http`, etc.) valid for GitHub rendering.
- Tables and callouts render correctly in any Markdown viewer.
```
